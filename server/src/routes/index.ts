...
apiRouter.get('/projects', async (_req, res) => { try { const result = await getProjects(); res.set('Cache-Control', 'public, max-age=60, s-maxage=300, stale-while-revalidate=600').json({ data: result.projects, source: result.source }); } catch { res.status(200).json({ data: localProjects(), source: 'local-fallback' }); } });
